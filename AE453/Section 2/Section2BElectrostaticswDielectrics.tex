...
\end{enumerate}
\newline
We found that the field of a polarized object is identival to the field produced by a distribution of "bound charges" or as we call it, "polarization" charges, $\rho_p$, $\sigma_p$ such that
\begin{equation*}
\begin{aligned}
\sigma_p &= \vv{P} \cdot \hat{n} \\
\rho_p &= -\nabla \cdot \vv{P} \\
\end{aligned}
\end{equation*}
Where $\vv{P}$ is the dipole moment per unit volume. "Dipole" refers to the separation of positive and negative charges.
\newline \newline
The strength of the polarization, (or how polarized a molecule is) is called the "Dipole Moment". It is expressed such that...
\begin{equation*}
\begin{aligned}
p = q \, d
\end{aligned}
...
\begin{equation*}
\begin{split}
q &= \text{Amount of charge} \\
d &= \text{Displaced distance}\\ \\
\end{split}
\end{equation*}
\newpage
$\vv{P}$ is the sum of dipole moments of all atoms/molecules
\newline
\begin{equation*}
\begin{split}
\vv{P} = N \, \vv{p_m}
...
\end{equation*}
\newline
Polarization of a material results in "bound" or "polarization" charges.
\begin{center}
\vspace{50mm}
\textbf{Figure 9}
\end{center}
Figure 9 shows that:
\begin{itemize}
\item A net charge at the end, at the surface, this is "bound" charge or as we will refer to it, "Polarization" charge. This is $\sigma_p$. 
\item $\rho_p$ comes about due to $\nabla \cdot \vv{P}$, for example, in an inhomogeneous material. This $\vv{P}$ field shows us that polarization is dependent on the material.
\end{itemize}
\begin{equation*}
\begin{split}
&\vv{P} = \chi (E) \, \vv{E} \\ \\
&\text{Electric Displacement, } \qquad \vv{D} = \epsilon_0 \, \vv{E} + \vv{P} \qquad \text{and } \qquad \nabla \cdot \vv{D} = \rho_f
\end{split}
\end{equation*}
\newline
Which is a useful way to express Gauss' law since it only depends on free charge, $\rho_f$. Often times, we don't care about "polarization" charges (we don't care about what is going on inside the material). Typically, we know $\rho_f$ but we don't know $\rho_p$
\begin{framed}
\newpage
\textbf{Example:} A wire carrying line charge $\lambda \, \big[\frac{c}{m}\big]$ is surrounded by an insulator with radius, a. Determine the Electric Displacement, $\vv{D}$.
\begin{center}
\vspace{30mm}
\textbf{Figure 10}
\end{center}
All we need to calculate $\vv{D}$ is the free charge. The free charge is in the wire and we also have some polarization charge in the insulator. A cylindrical surface with length, L, and radius, S. Using Gauss' Law in integral form, we get...
\newline
\begin{equation*}
\begin{aligned}
\oint_S \vv{D} \cdot \, \mathrm{d}\vv{A} = \int_{\volume} \rho_f \, \mathrm{d}\volume \quad \Rightarrow \quad D \, (2 \, \pi \, s \, L) = \lambda \, L \\ \\
\end{aligned}
\end{equation*}
Solving for D and then including the radial direction vector, $\hat{s}$, we get
\begin{equation*}
\begin{aligned}
\vv{D} = \frac{\lambda}{2 \, \pi \, s} \, \hat{s}
\end{aligned}
\end{equation*}
which is true inside and outside of the insulator. 
\newline \newline
Now let's say we have a free charge moving around outside the surface. What is the force acting on that charge. Now we need to know the electric field. 
\newline
\begin{equation*}
\begin{aligned}
\vv{F}_q = q \, \vv{E}
...
\end{equation*}
\newline
From Equation 34 which states $\vv{D} = \epsilon_0 \vv{E} + \vv{P}$ and since we are outside the insulator $(s>a)$ meaning $\vv{P} = 0$ therefore...
\newline
\begin{equation*}
\begin{aligned}
\vv{E} = \frac{1}{\epsilon_0} \, \vv{D} = \frac{\lambda}{2 \, \pi \, \epsilon_0 \, s} \, \hat{s}
\end{aligned}
\end{equation*}
\newline
Finally, what is the electric field inside the insulator. Normally, this is not needed but there is a chance that the electric field is so strong that it breaks the insulator. In this situation we must know $\vv{P}$ since it is no longer equal to zero. As a result, we must know the susceptibility of the insulator, $\chi$ which is dependent on the material (Equation 27).
\end{framed}
\newpage
